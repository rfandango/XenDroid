...
static void ExceptionHandlerCallback(int signal_number, siginfo_t* signal_info,
  ...
  switch (signal_number) {
    case SIGSEGV:
      previous_handler = &original_sigsegv_handler_;
      break;
    case SIGBUS:
      previous_handler = &original_sigbus_handler_;
      break;
    case SIGILL:
      previous_handler = &original_sigill_handler_;
      break;
  }
  if (previous_handler) {
    ...
    if (!(previous_handler->sa_flags & SA_SIGINFO) &&
        previous_handler->sa_handler &&
        previous_handler->sa_handler != SIG_DFL &&
