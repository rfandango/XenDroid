...
class GuestScheduler {
 ...
 private:
  ...
  struct Cpu {
    ...
    // Atomic only so YieldCurrentThread's fast path can test it without
    // taking lock_; every mutation still happens under the lock, so relaxed
    // ordering is enough. A stale zero there costs one skipped yield, which
    // the next call or the quantum watchdog picks up.
  ...
  // Clears one priority bit of a CPU's ready summary. Callers hold lock_; the
