...
object SettingDescriptions {
    val byName: Map<String, String> = mapOf(
        ...
        "vulkan_vrs_blended" to "Shades part of the frame at a coarser rate to save GPU time, often a large speed-up in shading-bound games. Coarser rates are faster and blockier. On most devices 4x2 and 4x4 fall back to 2x2 whenever the game is using multisampling",
        "vulkan_vrs_scope" to "Narrows the coarse rate to part of the frame. Every draw is the fastest and is the default; UI and text are kept sharp by the shader cutoff below rather than by this. Set both to off to coarsen the whole frame",
        "vulkan_vrs_min_shader_dwords" to "Leaves simple draws at the native rate. Coarse shading only saves work inside the pixel shader, so text, UI and plain blits gain nothing from it and only look blockier. Raise this if UI still looks coarse, lower it for more speed",
